...
import {
  Plus, Edit, KeyRound, Users, ShieldCheck, FlaskConical, MessageSquare,
} from 'lucide-react';
...
import EnviarMensajeModal from '../../components/users/EnviarMensajeModal';
...
export default function UsersList() {
  ...
  const [userParaMensaje, setUserParaMensaje] = useState(null);
  ...
  const openEdit = (user) => { setUserSeleccionado(user); setShowEditModal(true); };
  const openReset = (user) => { setUserSeleccionado(user); setShowResetModal(true); };
  const openMensaje = (user) => { setUserParaMensaje(user); setShowMensajeModal(true); };

  if (loading) return <LoadingSpinner message="Cargando usuarios..." />;

  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <PageHeader icon={Users} title="Gestión de Usuarios" subtitle="Administra los usuarios del sistema">
      ...
      <div className="grid grid-cols-3 gap-4">
        <StatCard icon={Users}      label="Total usuarios"   value={users.length}                                   variant="green" />
        <StatCard icon={ShieldCheck} label="Administradores" value={users.filter(u => u.role === 'ADMIN').length}   variant="green" />
        <StatCard icon={FlaskConical} label="Analistas"      value={users.filter(u => u.role === 'ANALYST').length} variant="gold"  />
      </div>
                ...
                <tr key={user.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-center">
                    <div className="flex justify-center gap-2">
                      <IconButton icon={Edit}          onClick={() => openEdit(user)}    title="Editar usuario"      variant="green" />
                      <IconButton icon={KeyRound}      onClick={() => openReset(user)}   title="Resetear contraseña" variant="gold"   />
                      <IconButton icon={MessageSquare} onClick={() => openMensaje(user)} title="Enviar mensaje"      variant="green" />
                    </div>
                  </td>
                </tr>
              );
      ...
      <EnviarMensajeModal
        ...
        onClose={() => { setShowMensajeModal(false); setUserParaMensaje(null); }}
        user={userParaMensaje}
      />
    </div>
  );
}
