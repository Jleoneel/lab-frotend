...
import {
  Plus, Search, FlaskConical, AlertTriangle, Package,
  TrendingDown, TrendingUp, Edit, Filter, ChevronDown, CheckCircle, Box,
} from 'lucide-react';
...
export default function ReactivosList() {
  ...
  const openEdit = (reactivo) => { setReactivoSeleccionado(reactivo); setShowEditModal(true); };
  const openIngreso = (reactivo) => { setReactivoSeleccionado(reactivo); setShowIngresoModal(true); };
  ...
  const stockBajo = reactivos.filter((r) => r.stockActual <= r.stockMinimo && r.stockMinimo > 0);
  const stockPromedio = Math.round(
    reactivos.reduce((acc, r) => acc + r.stockActual, 0) / (reactivos.length || 1),
  );
  const totalMovimientos = reactivos.reduce((acc, r) => acc + (r._count?.movimientos || 0), 0);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      >
        <div className="flex flex-wrap gap-2">
          <button
            ...
            style={{ borderColor: '#DC262640', color: '#DC2626', backgroundColor: '#FFFFFF' }}
            onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = '#FEF2F2'; e.currentTarget.style.borderColor = '#DC2626'; }}
            onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = '#FFFFFF'; e.currentTarget.style.borderColor = '#DC262640'; }}
          >
            <TrendingDown className="w-4 h-4" />
            Registrar Consumo
          </button>
          <button
            onClick={() => { setReactivoSeleccionado(null); setShowIngresoModal(true); }}
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm font-medium transition-all hover:shadow-md"
            style={{ borderColor: '#00993340', color: '#009933', backgroundColor: '#FFFFFF' }}
            onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = '#E8F5E9'; e.currentTarget.style.borderColor = '#009933'; }}
            onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = '#FFFFFF'; e.currentTarget.style.borderColor = '#00993340'; }}
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <StatCard icon={Package}       label="Total reactivos"     value={reactivos.length}    variant="green" />
        <StatCard icon={Box}           label="Stock promedio"      value={stockPromedio}         variant="green" />
        <StatCard icon={TrendingDown}  label="Movimientos"         value={totalMovimientos}      variant="gold"  />
        <div
        ...
        >
          <div className="flex items-center gap-3">
            <div
              ...
              style={{ backgroundColor: stockBajo.length > 0 ? '#FEF2F2' : '#F5F5F5' }}
            >
              <AlertTriangle
                className="w-5 h-5"
            ...
            <div>
              <p className="text-xs" style={{ color: '#666666' }}>Stock bajo mínimo</p>
              <p
        ...
        >
          ...
          <div className="flex-1">
            ...
            <p className="text-sm mt-1" style={{ color: '#666666' }}>
              {stockBajo.map((r) => `${r.nombre} (${r.stockActual} ${UNIDADES[r.unidad]})`).join(' · ')}
            </p>
          </div>
        </div>
      )}
      ...
      {filteredReactivos.length === 0 ? (
        <EmptyState
          icon={FlaskConical}
          title={searchTerm || categoriaFiltro ? 'No se encontraron reactivos' : 'No hay reactivos registrados'}
          description={
        ...
        <div className="bg-white rounded-2xl border shadow-sm overflow-hidden" style={{ borderColor: '#E5E5E5' }}>
          ...
          <div className="hidden lg:block overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="border-b" style={{ backgroundColor: '#F9F9F9', borderColor: '#E5E5E5' }}>
                <tr>
                  {['Reactivo', 'Categoría', 'Stock actual', 'Stock mínimo', 'Estado', 'Acciones'].map((h, i) => (
                    <th
                      key={h}
              ...
              <tbody className="divide-y" style={{ borderColor: '#E5E5E5' }}>
                {filteredReactivos.map((reactivo) => {
                  const bajo = reactivo.stockActual <= reactivo.stockMinimo && reactivo.stockMinimo > 0;
                  return (
                    <tr key={reactivo.id} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="font-medium" style={{ color: '#009933' }}>{reactivo.nombre}</div>
                        <div className="text-xs font-mono" style={{ color: '#666666' }}>{reactivo.codigo}</div>
                      </td>
                      <td className="px-6 py-4">
                        <span
                      ...
                      <td className="px-6 py-4 text-right font-mono font-medium" style={{ color: bajo ? '#DC2626' : '#009933' }}>
                        {reactivo.stockActual}{' '}
                        <span className="text-xs" style={{ color: '#666666' }}>{UNIDADES[reactivo.unidad]}</span>
                      </td>
                      <td className="px-6 py-4 text-right font-mono" style={{ color: '#666666' }}>
                        {reactivo.stockMinimo}{' '}
                        <span className="text-xs">{UNIDADES[reactivo.unidad]}</span>
                      </td>
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex justify-center gap-2">
                          <IconButton icon={Edit}       onClick={() => openEdit(reactivo)}    title="Editar"           variant="green" />
                          <IconButton icon={TrendingUp} onClick={() => openIngreso(reactivo)} title="Registrar ingreso" variant="green" />
                        </div>
                      </td>
                    </tr>
                  );
          ...
          <div className="lg:hidden divide-y" style={{ borderColor: '#E5E5E5' }}>
            {filteredReactivos.map((reactivo) => {
              const bajo = reactivo.stockActual <= reactivo.stockMinimo && reactivo.stockMinimo > 0;
              return (
                <div key={reactivo.id} className="p-4 hover:bg-gray-50/50 transition-colors">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      <h3 className="font-semibold" style={{ color: '#009933' }}>{reactivo.nombre}</h3>
                      <p className="text-xs font-mono mt-0.5" style={{ color: '#666666' }}>{reactivo.codigo}</p>
                    </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-3">
                    <div className="bg-gray-50 rounded-lg p-2 text-center">
                      <p className="text-xs" style={{ color: '#666666' }}>Categoría</p>
                      <span
                    ...
                    <div className="bg-gray-50 rounded-lg p-2 text-center">
                      <p className="text-xs" style={{ color: '#666666' }}>Estado</p>
                      <div className="mt-1"><StockBadge bajo={bajo} /></div>
                    </div>
                  </div>
                  <div className="flex justify-between items-center pt-2 border-t" style={{ borderColor: '#E5E5E5' }}>
                    <div>
                      ...
                      <p className="font-mono font-medium" style={{ color: bajo ? '#DC2626' : '#009933' }}>
                        {reactivo.stockActual}{' '}
                        <span className="text-xs">{UNIDADES[reactivo.unidad]}</span>
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="text-xs" style={{ color: '#666666' }}>Stock mínimo</p>
          ...
          <div className="border-t px-6 py-3 flex justify-between items-center text-sm" style={{ backgroundColor: '#F9F9F9', borderColor: '#E5E5E5' }}>
            ...
            <span className="font-medium" style={{ color: '#009933' }}>
              Total unidades: {reactivos.reduce((acc, r) => acc + r.stockActual, 0)}
            </span>
          </div>
        </div>
      )}

      <NuevoReactivoModal  isOpen={showNuevoModal}   onClose={() => setShowNuevoModal(false)}   onSaved={loadReactivos} />
      <EditReactivoModal   isOpen={showEditModal}    onClose={() => setShowEditModal(false)}    reactivo={reactivoSeleccionado} onSaved={loadReactivos} />
      <RegistrarIngresoModal isOpen={showIngresoModal} onClose={() => setShowIngresoModal(false)} reactivo={reactivoSeleccionado} onSaved={loadReactivos} />
      <RegistrarConsumoModal isOpen={showConsumoModal} onClose={() => setShowConsumoModal(false)} analysis={null} onSaved={loadReactivos} />
    </div>
  );
}
