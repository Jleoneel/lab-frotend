...
import {
  Plus, FileText, Eye, Search, ArrowUpDown, Calendar,
  DollarSign, Users, XCircle, Clock, Send, ThumbsUp, RefreshCw,
} from 'lucide-react';
...
export default function QuotesList() {
  ...
  const filteredQuotes = quotes
    ...
    .sort((a, b) => {
      ...
      else if (sortField === 'createdAt') { aVal = new Date(aVal).getTime(); bVal = new Date(bVal).getTime(); }
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <PageHeader icon={FileText} title="Cotizaciones" subtitle="Gestiona todas tus cotizaciones">
        <Link to="/quotes/new">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={FileText}   label="Total"      value={quotes.length}                                              variant="green" />
        <StatCard icon={ThumbsUp}   label="Aprobadas"  value={quotes.filter(q => q.status === 'APPROVED').length}         variant="green" />
        <StatCard icon={RefreshCw}  label="Convertidas" value={quotes.filter(q => q.status === 'CONVERTED').length}       variant="gold"  />
        <StatCard icon={DollarSign} label="Monto total" value={formatCurrency(totalMonto)}                                variant="gray"  />
      </div>
          ...
          <div className="hidden lg:block overflow-x-auto">
            <table className="w-full">
              <thead className="border-b" style={{ backgroundColor: '#F9F9F9', borderColor: '#E5E5E5' }}>
                <tr>
                  {['N° Cotización', 'Cliente', 'Fecha', 'Total', 'Estado', 'Acciones'].map((h, i) => (
                    <th
                      key={h}
                  ...
                  return (
                    <tr key={quote.id} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="font-mono text-sm font-medium" style={{ color: '#333333' }}>
                          {quote.quoteNumber}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm" style={{ color: '#666666' }}>{quote.client}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          <Calendar className="w-4 h-4" style={{ color: '#666666' }} />
                          <span className="text-sm" style={{ color: '#666666' }}>{formatDate(quote.createdAt)}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
