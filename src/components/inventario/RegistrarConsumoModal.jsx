...
import { reactivoService, UNIDADES, RAZONES } from '../../services/reactivoService';
import Swal from 'sweetalert2';

export default function RegistrarConsumoModal({ isOpen, onClose, analysis, onSaved }) {
  const [reactivos, setReactivos] = useState([]);
  const [consumos, setConsumos] = useState([{ reactivoId: '', cantidad: '', razon: 'ANALISIS', observaciones: '' }]);
  ...
  const addConsumo = () => {
    setConsumos([...consumos, { reactivoId: '', cantidad: '', razon: 'ANALISIS', observaciones: '' }]);
  };
  ...
  const handleSave = async () => {
    const invalid = consumos.some(c => !c.reactivoId || !c.cantidad || parseFloat(c.cantidad) <= 0);
    if (invalid) {
      await Swal.fire({ icon: 'warning', title: 'Datos incompletos', text: 'Selecciona reactivo y cantidad en todos los registros', confirmButtonColor: '#009933' });
      return;
    }
    ...
    try {
      ...
      setConsumos([{ reactivoId: '', cantidad: '', razon: 'ANALISIS', observaciones: '' }]);
      onSaved?.();
      onClose();
    } catch (error) {
      await Swal.fire({
  ...
  const inputStyle = {
    width: '100%', padding: '6px 10px', border: '1px solid #E5E5E5',
    borderRadius: '10px', fontSize: '13px', color: '#333333', outline: 'none'
  };
      ...
      <div className="space-y-4">
        ...
        {analysis && (
          <div className="px-4 py-3 rounded-xl flex items-center gap-3"
            ...
            <FlaskConical className="w-4 h-4 flex-shrink-0" style={{ color: '#009933' }} />
            <div>
        ...
        <div className="space-y-3 max-h-[50vh] overflow-y-auto pr-1">
          {consumos.map((consumo, index) => {
            ...
            return (
              <div key={index} className="p-4 rounded-xl border space-y-3"
                ...
                <div className="flex items-center justify-between">
                  ...
                  {consumos.length > 1 && (
                    <button onClick={() => removeConsumo(index)}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-medium mb-1" style={{ color: '#666666' }}>Reactivo *</label>
                    <select value={consumo.reactivoId}
                  ...
                  <div>
                    ...
                    <input type="number" value={consumo.cantidad}
                      onChange={(e) => updateConsumo(index, 'cantidad', e.target.value)}
                      style={inputStyle} min="0.001" step="0.001" placeholder="0.000"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-medium mb-1" style={{ color: '#666666' }}>Razón</label>
                    <select value={consumo.razon}
                      onChange={(e) => updateConsumo(index, 'razon', e.target.value)}
                      style={{ ...inputStyle, appearance: 'auto' }}>
                      {Object.entries(RAZONES).map(([key, label]) => (
                  ...
                  <div>
                    <label className="block text-xs font-medium mb-1" style={{ color: '#666666' }}>Observaciones</label>
                    <input value={consumo.observaciones}
                ...
                {/* Alerta stock bajo */}
                {reactivoActual && parseFloat(consumo.cantidad) > reactivoActual.stockActual && (
