...
export default function Navbar() {
  ...
  const handleNotifClick = async (notif) => {
    ...
    else if (notif.tipo === 'CALIBRACION_PROXIMA' || notif.tipo === 'CALIBRACION_VENCIDA') navigate('/equipos');
    else if (notif.tipo === 'MENSAJE') {
      try {
        await mensajeService.marcarLeido(notif.mensajeId);
        const todos = await mensajeService.getAll();
        const completo = todos.find((m) => m.id === notif.mensajeId);
        setMensajeAbierto({ ...notif, mensaje: completo?.contenido || notif.mensaje });
      } catch (e) {
        console.error(e);
      }
    }
  ...
  return (
    <>
      ...
      >
        <div className="h-full px-6 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            {/* Dark mode */}
            <button
              onClick={() => { setIsDarkMode(!isDarkMode); document.documentElement.classList.toggle('dark'); }}
            ...
            <div className="relative">
              ...
              {showUserMenu && (
                <UserMenuDropdown
                  ...
                  onSettings={() => { setShowUserMenu(false); navigate('/settings'); }}
                  onChangePassword={() => { setShowUserMenu(false); setShowPasswordModal(true); }}
      ...
      <div className="h-16" />
      <MensajeModal mensaje={mensajeAbierto} onClose={() => setMensajeAbierto(null)} />
